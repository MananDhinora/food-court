...
def signup():
    ...
    user = {"name": user_data["name"], "email": user_data["email"], "password": hashed_password, "user-type": user_data["userType"]}
...
def get_food():
    ...
    foods = mongo.food_collection.find()
    final_foods = []
    for food in foods:
        final_foods.append({"_id": str(food["_id"]), "name": food["name"],"image": food["image"], "price": food["price"], "canteen": food["canteen"]})
    return final_foods, 200
